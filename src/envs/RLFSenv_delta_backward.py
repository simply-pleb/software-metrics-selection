...
class RLFSEnvDeltaBackward:
    ...
    def reset(self):
        ...
        self.state = np.ones(self.state_size, dtype=bool)
        self._state_prev_error = self.error_f(self.data, self.state)  # error_f(data, self.state)
        self.cur_num_features = 0
        return self.state
