...
if __name__ == "__main__":
    ...
    env = RLFSEnvFull(state_size=state_space, data=X_train, max_features=state_space//2)
    agent = DQN.DQNAgent(state_space, action_space, device, gamma=0, lr=0.0005)
    
    if LOAD is None:
        episode_returns = agent.train(env=env, num_episodes=10000, max_iters=state_space//2)
        torch.save(agent.target_net.state_dict(), "models/DQN/6/dqn_agent_target_weights.pth")
        torch.save(agent.policy_net.state_dict(), "models/DQN/6/dqn_agent_policy_weights.pth")
    ...
    INF_LOOP_CNT = 5
    env = RLFSEnvDeltaForward(state_size=state_space, data=X_train, max_features=state_space)
    errors = []
    num_ftrs = []
    print("waiting...")
