...
if __name__ == "__main__":
    ...
    agent = REINFORCE.REINFORCEAgent(state_space, action_space, gamma=0.999, lr=0.0025)
    if LOAD is None:
        for num_features in range(1, 10):
            env = RLFSEnvSparse(state_size=state_space, data=X_train, max_features=num_features)    
            episode_returns = agent.train(env=env, num_episodes=3000, max_steps=num_features+5)
